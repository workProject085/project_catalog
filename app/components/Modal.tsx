...
const ButtonWithModal = ({ data }: any) => {
  ...
  return (
    <>
      <button className='bg-[#30222d] text-white border-none p-2 w-full cursor-pointer mt-1' onClick={() => setIsOpen(true)}>
        Ətraflı
      </button>
      <FullScreenMode url={data.images[currentSlideIndex]} isOpen={isOpenFS} setIsOpen={setIsOpenFS} />
              ...
              >
                <Dialog.Panel className='w-full h-full transform overflow-hidden rounded-2xl bg-white px-[10px] md:px-[60px] py-[24px] text-left shadow-xl transition-all'>
                  <button
                    ...
                    onClick={onClose}
                  >
                    <Image priority src={'close.svg'} alt='close' width={24} height={24} />
                  </button>
                  <button
                    type='button'
                    ...
                    onClick={onOpenFS}
                  >
                    <Image priority src={'enlarge.svg'} alt='close' width={24} height={24} />
                  </button>
                  <Slider previews={data.images.map((image: any) => `/${image}`)} onSelectSlide={onSelectSlide}>
                    {data.images?.map((image: any, index: any) => (
                      <div className='w-full h-[calc(100vh-180px)] border flex-1' key={index}>
